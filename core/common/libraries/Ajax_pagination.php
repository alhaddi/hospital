...
class Ajax_pagination{
	...
	var $uri_segment		= 3;
	var $full_tag_open		= "<ul  class='pagination pull-right' style='margin:0; padding:0;'>";
   ...
   var $show_count      = true;
	...
	function create_links()
	{
      ...
      if ($this->show_count)
      {
		...
		$return .= $label;
		$return .= '</div>';
		$return .= '<div class="col-md-9 col-xs-12">';
		$return .= $output;
		$return .= '</div>';
		$return .= '</div>';
		return $return;		
	...
	function getAJAXlink( $count, $text) {
        ...
        $this->additional_param = "{'page' : $pageCount,'total_count' : $this->total_rows }";

		return "<a href=\"javascript:void(0);\"
		         " . $this->anchor_class . "
					onclick=\"filter('". $this->base_url . intval($count) ."'); return false;\">"
				. $text .'</a>';
	}
