...
class FPDF_AutoWrapTable{
	...
	public function rptDetailData () {
		...
		$this->fpdf->SetXY($left += 23,$this->fpdf->getY()); $this->fpdf->Cell(23, $h, 'PPH', 1, 0, 'C');
...
$this->fpdf = new fpdf('P','mm',array(297,210));
$this->fpdf->SetMargins(10,20,10,0);


foreach($query1 as $row){

$query2 = $this->Pendukung_rencana_pengeluaran_model->data_pdf2('',$row['tgl_blud'],$row['no_cek']);
$data = array();

foreach($query2 as $r){
	$jenis_biaya = $this->db->select('nama_anggaran as jenis_biaya')
		->where('id',$r['parent_id'])
		->get('anggaran')->row_array();

	$field = array();
	$field['nama_anggaran'] = $r['nama_anggaran']; 
	$field['jenis_biaya'] = $jenis_biaya['jenis_biaya'];
...
$tabel = new FPDF_AutoWrapTable($this->fpdf, $data, $options, array($row['tgl_blud'],$row['no_cek'],$tgl_cetak));
$tabel->printPDF();
