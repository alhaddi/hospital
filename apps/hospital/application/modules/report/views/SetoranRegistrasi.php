...
<div style="padding: 20px">
	<div class="row">
		<div class="col-sm-3">
			...
			<select class="form-control" name="id_cara_bayar"  id="id_cara_bayar3" data-datatable-filter="true" data-datatable-id='<?=$id_table3?>'>
		...
		<div class="col-sm-3"> 
			...
			<div class="col-sm-6">
				<button onclick="pdf_loket()" class="btn btn-xs btn-success form-control"><span class="fa fa-print"></span> &nbsp; Pdf File </button>
			</div>
		</div>
	</div>
</div>
...
//script yg di tambah sm fatur
function pdf_loket(){
	var d1=$("#setoran_loket_daterange1").val();
	var d2=$("#setoran_loket_daterange2").val();
	var tgl1=d1.split("/");
	var tgl2=d2.split("/");
	...
	window.open('<?=base_url()?>report/showLoket?tgl1='+t1+'&tgl2='+t2+'&cara_bayar='+id_cara_bayar3+'&column_order3='+column_order3+'&dir_order3='+dir_order3);
}
