<?php
	...
	*/
	...
	class Billing_tindakan extends MY_Controller
	{
		...
		{
			$data['data']['id_table'] = 'billing_tindakan';
			$data['data']['datatable_list'] = 'billing_tindakan/ajax_list';
			$data['data']['datatable_save'] = 'billing_tindakan/ajax_save';
			
			$data['data']['id_table2'] = 'billing_penunjang';
			$data['data']['datatable_list2'] = 'billing_tindakan/ajax_list2';
			$data['data']['datatable_save2'] = 'billing_tindakan/ajax_save2';
   ...
   public function ajax_list_2()
	{	
		...
		{	
			...
			$no = $_POST['start'];
			foreach ($list as $row) {
				$cek=$this->db->query("SELECT id,`status`,id_komponen,id_penunjang FROM trs_billing WHERE id_appointment='$row->id_appointment' AND id_komponen = 4")->row();
				$jenis_appointment="Tindakan";
				if(!empty($cek->id)){
		...
		{	
			...
			$no = $_POST['start'];
			foreach ($list as $row) {
				$cek=$this->db->query("SELECT id,`status`,id_komponen,id_ms_penunjang FROM trs_billing WHERE id_appointment='$row->id_appointment' AND id_komponen = 5")->row();
				$keterangan="-";
				if(!empty($cek->id)){
					if($cek->status == '1'){
					$label='<i class="label label-success">Sudah Bayar</i>';
					$action='<button type="button" onclick="window.open(\' '.site_url('billing_konsultasi/kwitansi/'.$row->id_pasien.'/'.$cek->id).' \')" target="_BLANK" class="btn btn-default"  title="Ubah data"><i class="fa fa-print"></i> Kwitansi</button>';
					}
					else{
					$label='<i class="label label-danger">Belum Bayar</i>';
					$action='<button type="button" onclick="addmodalpenunjang2('.$row->id.','.$cek->id.')" class="btn btn-default"> <i class="flaticon-clinic-history-medical-paper-on-clipboard"></i> Penunjang</button>';
					}
					
					$keterangan=get_field($cek->id_ms_penunjang,'ms_penunjang');
				}else{
					$label='<i class="label label-warning">-</label>';
					$action='<button type="button" onclick="addmodalpenunjang('.$row->id.')" class="btn btn-default"> <i class="flaticon-clinic-history-medical-paper-on-clipboard"></i> Penunjang</button>';
				}
				$no++;
				$fields = array();
...
echo '<center><button type="button" class="btn btn-danger" data-dismiss="modal">Tutup</button></center>';
		...
		public function form_penunjang2($id,$id_billing="")
		{
			...
			$data['penunjang'] = $this->db->query("SELECT a.*,b.nama,c.jumlah as jumlahp,c.biaya as biayap,c.klinis FROM trs_billing a,ms_penunjang b,trs_penunjang c WHERE a.id_ms_penunjang=b.id AND a.id_penunjang=c.id AND a.id='$id_billing'")->result_array();

			$this->load->view('form_penunjang_edit',$data);
		}
		...
		public function form_penunjang($id,$ket="")
		{
			$data['ket'] = $ket;
			$data['title'] = 'Konsultasi';
			$data['id_table'] = 'konsultasi';
			...
			$this->load->view('form_penunjang',$data);
		}
		...
		public function ajax_save2()
		{
			...
			if(!empty($post['penunjang']))
			{
				$value = $post['penunjang'];
					...
					if($value['biaya'] != '' AND $value['biaya'] > 0){
					$penunjang['id_ms_penunjang'] = $value['id_ms_penunjang'];
					$penunjang['biaya'] = rupiah_to_number($value['biaya']);
					$penunjang['id_konsultasi'] = element('id',$post);
					$penunjang['id_pasien'] = $pasien['id_pasien'];
					$penunjang['id_dokter'] = $value['id_dokter'];
					$penunjang['jumlah'] = 1;
					$penunjang['klinis'] = $value['klinis'];
					
					$total=$penunjang['biaya'];
					$this->db->insert('trs_penunjang',$penunjang);
					$i['id_penunjang'] = $this->db->insert_id();
					$i['id_ms_penunjang'] = $value['id_ms_penunjang'];
					}		
			}
