...
class FPDF_AutoWrapTable{
	...
	public function rptDetailData () {
		...
		$this->fpdf->MultiCell(0, 3, 'BUKU RINCIAN OBJEK BELANJA',0,'C');
		...
		$this->fpdf->MultiCell(0, 3, 'BENDAHARA PENGELUARAN',0,'C');
		...
		$this->fpdf->SetXY($left += 18,$this->fpdf->getY()); $this->fpdf->Cell(14, $h, '', 1, 0, 'C');

		$this->fpdf->SetXY($left += 14,$this->fpdf->getY()); $this->fpdf->Cell(50, $h, '', 1, 0, 'C');

		$this->fpdf->SetXY($left += 50,$this->fpdf->getY()); $this->fpdf->Cell(23, $h, '', 1, 0, 'C');

		$this->fpdf->SetXY($left += 23,$this->fpdf->getY()); $this->fpdf->Cell(23, $h, '', 1, 0, 'C');

		$this->fpdf->SetXY($left += 23,$this->fpdf->getY()); $this->fpdf->Cell(23, $h, '', 1, 0, 'C');

		$this->fpdf->SetXY($left += 23,$this->fpdf->getY()); $this->fpdf->Cell(32, $h, rupiah($this->query3[7]), 1, 0, 'R');
		...
		$jmlGu = 0;
		$jmlTup = 0;
		$jmlLsLalu = $this->query3[8];
		$jmlGuLalu = $this->query3[9];
		$jmlTupLalu = $this->query3[10];
		$jmlLsTotal = 0;
		$jmlGuTotal = 0;
		$jmlTupTotal = 0;
		...
		foreach ($this->data as $baris) {
			...
			$jmlGu += $baris['gu'];
			$jmlTup += $baris['tup'];
			$this->query3[7] -= $baris['ls']+$baris['gu']+$baris['tup'];
			...
			$this->fpdf->Cell(32,5,rupiah($this->query3[7]),1,0,'R');
			...
			$this->fpdf->SetXY($left+120,$this->fpdf->GetY()-3); $this->fpdf->Cell(0, 3, 'Garut, '.convert_tgl($this->query3[5],'d F Y',1),0,1,'L');
...
$tabel = new FPDF_AutoWrapTable($this->fpdf, $query2, $options, array($row['no_rekening'],$row['nama_anggaran'],$row['pagu'],$row['tgl_blud'],$row['last'],$tgl_cetak,$cek,$saldo,$ls,$gu,$tup));	
$tabel->printPDF();
