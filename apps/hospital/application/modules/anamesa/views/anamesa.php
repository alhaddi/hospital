  ...
  <div class="modal-dialog" role="document" style="width:80%;">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Data Anamesa</h4>
      </div>
...
<script>
	function addmodal(id){
		...
		$.ajax({
			type: "POST",
			url: "<?=base_url()?>anamesa/addanamesa/"+id,
...
function excel(){
	var d1=$("#anamesa_daterange1").val();
	var d2=$("#anamesa_daterange2").val();
	var tgl1=d1.split("/");
	var tgl2=d2.split("/");
	...
	window.open('<?=base_url()?>anamesa/excel?tgl1='+t1+'00:00:00&tgl2='+t2+' 23:59:59&column_order='+column_order+'&dir_order='+dir_order);
}
function pdf(){
	var d1=$("#anamesa_daterange1").val();
	var d2=$("#anamesa_daterange2").val();
	...
	window.open('<?=base_url()?>anamesa/pdf?tgl1='+t1+' 00:00:00&tgl2='+t2+' 23:59:59&column_order='+column_order+'&dir_order='+dir_order);
}
	function modal_anamesa(id_anamesa){
		$.ajax({
			url		:"<?=site_url('anamesa/ajax_edit')?>",
			data	:{
				id:id_anamesa
			},
			type	:"post",
			...
			success	:function(jdata){
				...
				formz[0].reset();
				if(jdata.data){
					$.each(jdata.data,function(i,v){
						formz.find('[name="'+i+'"]').val(v);
					});
				}
				
				if(jdata.detail){
					$.each(jdata.detail,function(i,v){
						formz.find('[name="komponen['+v.id_ms_anamesa+']"]').val(v.hasil);
					});
				}
