					...
					>
						<thead>
							<tr>
								...
								<th data-datatable-align="text-left">No Cek</th>
								...
								<th data-datatable-align="text-left">Setoran</th>
								<th data-datatable-align="text-left">Kategori Belanja</th>
...
function modal_pengeluaran(id_blud){
		$.ajax({
			url		:"<?=site_url('pengeluaran/ajax_edit')?>",
			data	:{
				id:id_blud
			},
			type	:"post",
			...
			success	:function(jdata){
				...
				if(jdata){
					$.each(jdata,function(v,i){
						...
						{
							formz.find('[name="'+i.name+'"]').val(i.value);
							if(formz.find('select[name="'+i.name+'"]').data('plugin') == 'select2') {
								formz.find('select[name="'+i.name+'"]').select2();
							}
						}
