<?php
	...
	*/
	...
	class Pelayanan_rawat_inap extends MY_Controller
	{
		function __construct()
		{
			parent::__construct();
			$this->load->model('Pelayanan_rawat_inap_model');
...
echo '<center><button type="button" class="btn btn-danger" data-dismiss="modal">Tutup</button></center>';
		...
		{
			$post = $this->input->post(NULL,TRUE);
			if(!empty($post['penunjang']['id_ms_penunjang']))
			{
				...
				foreach($value['id_ms_penunjang'] as $index=>$val)
				{
					...
					$penunjang['id_pasien'] = $value['id_dokter'][$index];
					$penunjang['jumlah'] = 1;
