					...
					<div class="row">
						<div class="col-md-4">
							...
							<select class="form-control" name="id_cara_bayar"  id="id_cara_bayar" data-datatable-filter="true" data-datatable-id='<?=$id_table?>'>
								...
								$s=($f['id'] == 1)?'selected':'';
								?>
									<option <?=$s?> value="<?=$f['id']?>"><?=$f['nama']?></option>
								<?php } ?>
							</select>
						</div>
						<div class="col-md-4">
						...
						<div class="col-md-4">
							<label>Action</label>
							<br>
							<button onclick="pdf();" class='btn btn-danger' type='button'><i class='fa fa-pdf-o'></i> Cetak ke PDF</button> <a href="<?=site_url('report?rajal=ya')?>" class='btn btn-success'>Laporan Billing Rajal</a>
						</div>
					</div>
				</div>
			</div>
...
<form id="as<?=$id_table?>" action="<?=site_url('billing/saveeditpoli')?>" method="POST">
	<div id="as_<?=$id_table?>" class="modal fade in" role="dialog">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					<h4 class="modal-title" id="myModalLabel"><span id="modal_title"></span> Form Edit Billing</h4>
				</div>
				<!-- /.modal-header -->
				<div class="modal-body" id="as-addmodal">
				</div>
				<!-- /.modal-body -->
				<div class="modal-footer">
...
<script>
	function modal_edit_poli(id){
		
		$.ajax({
			type: "POST",
			url: "<?=base_url()?>billing/editpoli/"+id,
			success: function(data) {
				$("#as_<?=$id_table?>").modal('show');
				$("#as-addmodal").html(data);
			}
		});
		return false;
	}
