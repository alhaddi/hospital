...
class Billing_model extends MY_Model {
	...
	private function _get_datatables_query()
	{
		...
		{
			//$order = $this->order;
			$this->db->order_by("trs_billing_manual.status", "ASC");
	...
	function get_datatables($id)
	{
		...
		$query = $this->db->select('
			...
			trs_billing_manual.id_komponen,
			trs_billing_manual.add_time
		')
		->where_in('trs_billing_manual.id_komponen',array(4,5))
		->join('trs_appointment','trs_billing_manual.id_appointment = trs_appointment.id','inner')
	...
	function count_filtered($id)
	{
		...
		$query = $this->db->select('
			count(trs_billing_manual.id) as num_rows
		',false)
		->where('trs_billing_manual.id_komponen',4)
		->join('trs_appointment','trs_billing_manual.id_appointment = trs_appointment.id','inner')
		->join('ms_pasien','trs_appointment.id_pasien = ms_pasien.id','inner')
		->join('ms_cara_bayar','trs_appointment.id_cara_bayar = ms_cara_bayar.id','inner')
		->join('ms_poliklinik','trs_appointment.id_poliklinik = ms_poliklinik.id','inner')
