...
//      if( !data.key ){
	...
	_getInnerHtml: function() {
		...
		if ( data.icon ) {
			if (data.icon.charAt(0) === "http://www.eakroko.de/"){
				imageSrc = data.icon;
			}else{
		...
		if(!opts.imagePath) {
			$("script").each( function () {
				...
				if( this.src.search(_rexDtLibName) >= 0 ) {
					if( this.src.indexOf("http://www.eakroko.de/")>=0 ){ // issue #47
						opts.imagePath = this.src.slice(0, this.src.lastIndexOf("http://www.eakroko.de/")) + "/skin/";
					}else{
						opts.imagePath = "skin/index.html";
					}
